...
export default function PengaturanPage() {
  ...
  const loadData = () => {
    ...
    const savedConfig = getWaConfig()
    if (savedConfig) {
      setWaConfig(savedConfig)
    }

    // Call it once on mount
    checkLiveConnection(savedConfig)
  }
  ...
  // Authorization check via ACL managePengaturanWa or viewPengaturanWa
  const activeAcl = getStoredAcl().find((r) => r.role === currentRole)
  const hasAccess = currentRole === "Super Admin" || !!activeAcl?.permissions.viewPengaturanWa
  const canManage = currentRole === "Super Admin" || !!activeAcl?.permissions.managePengaturanWa
